...
export default function SupplierPaymentModal({ supplier, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    try {
      await api.post(`/suppliers/${supplier.id}/payments`, {
      ...
      <div className="w-full max-w-sm bg-surface-light dark:bg-surface-dark rounded-xl shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <p className="text-sm text-muted-light dark:text-muted-dark">
            We currently owe {supplier.full_name}{' '}
            <span className="font-semibold text-text-light dark:text-text-dark">
              ₹{parseFloat(supplier.outstanding_balance).toLocaleString('en-IN')}
            </span>
          </p>
          ...
          <div>
            <label className="block text-sm font-medium text-muted-light dark:text-muted-dark mb-1">Reference number (optional)</label>
            <input
