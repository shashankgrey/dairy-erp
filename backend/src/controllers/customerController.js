...
async function getPurchaseHistory(req, res) {
  const { page = 1, limit = 20, product_id, from_date, to_date } = req.query;

  const pageNum = Math.max(1, parseInt(page) || 1);
  const pageSize = Math.min(1000, Math.max(1, parseInt(limit) || 20));
  const offset = (pageNum - 1) * pageSize;
...
async function createCustomer(req, res) {
  ...
  if (existing.rows.length > 0) {
    ...
    const restored = await pool.query(
      `UPDATE customers
       SET active = true, full_name = $1, address = $2, email = $3, notes = $4
       WHERE id = $5
       RETURNING *`,
      [full_name, address || null, email || null, notes || null, match.id]
    );

    return res.status(201).json({ ...restored.rows[0], restored: true });
  }

  const result = await pool.query(
    `INSERT INTO customers (full_name, mobile_number, address, email, notes)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [full_name, mobile_number, address || null, email || null, notes || null]
  );

  res.status(201).json({ ...result.rows[0], restored: false });
}
...
async function updateCustomer(req, res) {
  ...
  const conflict = await pool.query(
    `SELECT id, full_name FROM customers WHERE mobile_number = $1 AND id != $2`,
  ...
  const result = await pool.query(
    `UPDATE customers
     SET full_name = $1, mobile_number = $2, address = $3, email = $4, notes = $5
     WHERE id = $6
     RETURNING *`,
    [full_name, mobile_number, address || null, email || null, notes || null, req.params.id]
  );
...
async function deleteCustomer(req, res) {
  const hasPurchases = await pool.query(
    `SELECT 1 FROM purchases WHERE customer_id = $1 LIMIT 1`,
    [req.params.id]
  );
...
async function restoreCustomer(req, res) {
  await pool.query('UPDATE customers SET active = true WHERE id = $1', [req.params.id]);
...
async function getMonthlyBill(req, res) {
  ...
  const items = await pool.query(
    `SELECT pr.name AS product, pr.unit, SUM(p.quantity) AS quantity,
        ROUND(SUM(p.total_amount) / SUM(p.quantity), 2) AS avg_price,
  ...
  res.json({
    ...
    paidThisMonth: parseFloat(paymentsResult.rows[0].total),
  });
}
